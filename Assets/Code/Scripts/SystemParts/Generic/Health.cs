...
public class Health : MonoBehaviour
{
    ...
    public int LastHealingTaken { get; private set; }
    public bool IsDead { get; private set; }
    public bool IsInvulnerable { get; private set; }

    private void Awake()
    {
        SetStartingMaxHealth(maxHealth);
        IsDead = false;
    }
    ...
    public void Heal(int healAmount)
    {
        ...
        // call OnHeal action
        LastHealingTaken = CurrentHealth - healthBefore;
        if (LastHealingTaken > 0)
        {
    ...
    public void TakeDamage(int damage, float time = 0f)
    {
        ...
        var healthBefore = CurrentHealth;
        // Modify damage value accordingly (damage-armor)
        var damageAfterMods = damage;
        CurrentHealth -= damageAfterMods;
